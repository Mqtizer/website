...
import thumbsGIF from '../../images/thumbs_up.gif'
...
export default function Contact({ open, onClose }: ContactProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setDone(true)
      // Set a timeout to show the thank you message
      setTimeout(() => {
        setDone(false)
        onClose()
      }, 1000)
  ...
  return (
    <Backdrop>
      <ModalWrapper>
        {done ? (
          <ThumbsUp>
            <img src={thumbsGIF} alt="thumbs up" />
            <span>Thank you! We will get back to you</span>
          </ThumbsUp>
        ) : (
          <>
